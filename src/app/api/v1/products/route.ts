...
import { validateBody, productSchema } from '@/lib/api-validation'
import type { Where } from 'payload'

export async function GET(request: NextRequest) {
  const apiKey = await validateApiKey(request)
  if (!apiKey) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  if (!hasScope(apiKey, 'products:read')) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  ...
  const result = await payload.find({ collection: 'products', where, page, limit, sort: '-createdAt', depth: 0 })
